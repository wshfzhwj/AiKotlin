...
1.  **代码分析**： 先检查现有的 `build.gradle.kts` 版本，确保依赖不冲突。
2.  **UI 生成**： 先提供 XML 布局代码，确保 ID 命名符合 camelCase 且具有语义化（如 `btn_submit_order`）。
3.  **ViewModel 编写**： 使用 `StateFlow` 暴露 UI 状态，使用 `SharedFlow` 处理一次性事件（如弹窗、跳转）。
4.  **性能核查**： 检查是否有主线程耗时操作，是否正确使用了 `viewModelScope`。

# 常用代码模板片段 (Prompt Trigger)
...
- 尽量不要使用过时的api 如`onBackPressed()`，使用 `OnBackPressedDispatcher`。
